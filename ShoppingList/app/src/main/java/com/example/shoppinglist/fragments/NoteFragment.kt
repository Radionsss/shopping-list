...
class NoteFragment : BaseFragment(),NoteAdapter.NoteListener {
    private lateinit var binding: FragmentNoteBinding
    private lateinit var noteDataLauncher: ActivityResultLauncher<Intent>
    ...
    private fun onNoteDataLauncher (){
        noteDataLauncher=registerForActivityResult(ActivityResultContracts.StartActivityForResult()){
    ...
    override fun onClickNew() {
        noteDataLauncher.launch(Intent(activity,NewNoteActivity::class.java))
    }
    ...
    override fun editItem(note:NoteItem) {
        val intent =Intent(activity,NewNoteActivity::class.java).apply {
            putExtra(NEW_NOTE_KEY,note)
        }
        noteDataLauncher.launch(intent)
    }

    companion object {
        const val NEW_NOTE_KEY="key_note_data"
        const val EDIT_STATE_KEY="edit_state_note"
        @JvmStatic
